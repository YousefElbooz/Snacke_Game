...
void Gamestate::Renderthesnacketable(sf::RenderTarget* target)
{
    for (int i = 0; i < 60; i++)
        for (int j = 0; j < 50; j++)
        {
            sprite1.setPosition((3600 / 2) - float(i) * 16, float((2000 / 2)) - float(j) * 16); target->draw(sprite1);
        }
    splayingtable.setPosition((3600 / 2) - 79 * 16, float((2000 / 2)) - 59 * 16); target->draw(splayingtable);
...
void Gamestate::Renderthesnackebody(sf::RenderTarget* target)
{
    for (int i = 0; i < num; i++)
    {
        if (i == 0)
        {
            ...
            case UP:
                head.loadFromFile("imageserterte/head_up.png");
                shead.setPosition(float(3600 / 2) - s[i].x * 16, float((2000 / 2)) - s[i].y * 16); target->draw(shead);
                break;
        ...
        else if (i == num - 1)
        {
            ...
            case 0:
                ...
                case 1:
                    tail.loadFromFile("imageserterte/tail_down.png");
                    stail.setPosition(float(3600 / 2) - s[i].x * 16, float(float((2000 / 2))) - s[i].y * 16); target->draw(stail);
                    break;
                case -1:
                    tail.loadFromFile("imageserterte/tail_up.png");
                    stail.setPosition(float(3600 / 2) - s[i].x * 16, float((2000 / 2)) - s[i].y * 16); target->draw(stail);
                    break;
                default:
                    break;
                }
        ...
        else
        {
            ...
            case 0:
                ...
                case 1:
                    ...
                    sbody.setPosition(float(3600 / 2) - s[i].x * 16, float(2000 / 2) - s[i].y * 16); target->draw(sbody);
                    break;
                case -1:
                    body.loadFromFile("imageserterte/body_vertical.png");
                    sbody.setPosition(float(3600 / 2) - s[i].x * 16, float(2000 / 2) - s[i].y * 16); target->draw(sbody);
...
{
    {
        sprite3.setPosition(float(3600 / 2) - f[0].x * 16, float(2000 / 2) - f[0].y * 16); target->draw(sprite3);
        sprite3.setPosition(float(3600 / 2) - f[1].x * 16, float(2000 / 2) - f[1].y * 16); target->draw(sprite3);
    }
}
...
void Gamestate::initPauseMenu()
{
    ...
    this->PmenuP = new PauseMenuState(*this->window, this->font, "PAUSED");
    this->PmenuD = new PauseMenuState(*this->window, this->font, "You Losed");
    this->PmenuP->addButton(1, static_cast <float>(this->window->getSize().x)/2, 400.f, "Continuoe");
    this->PmenuP->addButton(0, static_cast <float>(this->window->getSize().x)/2, 700.f, "Quit");
    this->PmenuD->addButton(1, static_cast <float>(this->window->getSize().x)/2, 400.f, "Continuoe");
...
{
    { sbomb.setPosition(float(3600 / 2) - b[0].x * 16, float(2000 / 2) - b[0].y * 16); target->draw(sbomb); }
    { sbomb.setPosition(float(3600 / 2) - b[1].x * 16, float(2000 / 2) - b[1].y * 16); target->draw(sbomb); }
}
